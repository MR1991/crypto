...
First we need to clean the data to make sure we do not start with bad data. Some coins have duplicate rows, for now we exclude these completely.
...
Some coins might have missing values. We check for each coin whether it is missing values after the first observed value, for now we remove all coins that have missing data as they are usually low marketcap.

```{r Missing values in timeseries}

    keep_coins <- df_marketdata  %>% 
                  group_by(name) %>% 
                  summarize(missings = n() - (last(date) - first(date) + 1)) %>% 
                  filter(missings == 0) %>% 
                  select(name)

    df_marketdata2 <- df_marketdata %>% filter(name %in% keep_coins$name)
